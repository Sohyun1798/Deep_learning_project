...
class BaselineCategoryClassifier(BaseClassifier):
    def __init__(self, words_embed=None, len_vocab=0, len_embed=0, out_channels=300,
        ...
        if self.cuda_device is None:
            self.convs = [nn.Conv1d(self.len_embed, out_channels, width, padding=(width//2))
            ...
            self.convs = [nn.Conv1d(self.len_embed, out_channels, width, padding=(width//2)).cuda(self.cuda_device)
                        for width in conv_widths]
